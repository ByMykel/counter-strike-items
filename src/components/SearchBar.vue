<template>
    <div class="sticky top-0 left-0 right-0 z-50 px-5 pt-3 pb-2 duration-300"
        ...
        <div class="relative">
            ...
            <div class="absolute right-0 flex items-center h-full px-3">
                <p v-if="total" class="px-2 text-sm text-white rounded-md bg-black-200">total <span class="font-semibold">{{
                    total
                }}</span></p>
                <div v-else class="w-20 h-6 px-2 rounded-md bg-black-200 animate-pulse" />
            </div>
